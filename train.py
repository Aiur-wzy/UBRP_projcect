...
#os.environ['MASTER_PORT'] = '20727'

def get_sequence(s,l,r):
    # Open the reference genome file
    fasta = pysam.FastaFile("/home/zhiyuan/My_DNAbert/finetune/hg19.fa.gz")

    sequence = fasta.fetch(s, l - 1,
                           r - 1)  # O-based, half-open, i,e. [start, end), the first position is 0.
    sequence = sequence.upper()  # convert all ATCG to uppercase.
    return sequence
...
@dataclass
class DataArguments:
    data_path: str = field(default="/home/zhiyuan/My_DNAbert/sample_data", metadata={"help": "Path to the training data."})
    kmer: int = field(default=-1, metadata={"help": "k-mer for input sequence. -1 means not using k-mer."})


@dataclass
class TrainingArguments(transformers.TrainingArguments):
    cache_dir: Optional[str] = field(default="/home/zhiyuan/My_DNAbert/result/cache")
    run_name: str = field(default="WangZhiyuan_DNAbert_run")
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self,
        ...
        for i in range(0,len(data)):
            try:
                texts.append((get_sequence(str(data[i][0]), int(data[i][1]), int(data[i][2]))))
                labels.append(int(data[i][3]))
            except:
                raise ValueError(str(data[i]) + str(len(data[i])))
                #raise ValueError(get_sequence(str(data[i][0]), int(data[i][1]), int(data[i][2])))
